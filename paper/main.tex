...
\begin{table}[t]
  ...
  Test-set macro-MAP@10 evaluation of A3--A5 checkpoints pending.
\end{table}

\textbf{Preliminary A3--A5 findings.}
Based on best-epoch validation micro-MAP@10 (see Table~\ref{tab:ablation}
footnote), all three remaining ablation dimensions show modest variation.
Circuit depth (A3) has negligible effect: $L=1$ (0.882), $L=2$ (0.879),
and $L=3$ (0.883) are within 0.4\% of each other, suggesting a single
entangling layer suffices to capture pairwise similarity structure at $N=8$.
Negative mining strategy (A4) is similarly insensitive on micro-MAP@10:
random (0.882) and class-pair mining (0.879) are statistically
indistinguishable at this metric, because the ``other'' class majority
dominates micro-MAP@10 regardless of negative selection.
Test-set macro-MAP@10 evaluation—which weights minority classes equally—is
needed to determine whether hard negatives provide a meaningful benefit for
the confusable class pairs; this evaluation is pending.
Qubit count (A5) shows a slight upward trend: $N=4$ (0.879), $N=8$ (0.879),
$N=12$ (0.882), consistent with a small Hilbert-space benefit at larger qubit
counts, though the differences remain within noise on micro-MAP@10.
Full test-set macro evaluation of all A3--A5 checkpoints is pending.
...
Validation micro-MAP@10 shows random mining (0.882) and domain-specific
class-pair mining (0.879) to be effectively indistinguishable, because
83.6\% ``other'' class queries dominate micro-MAP@10 regardless of negative
selection strategy.
The mining benefit is expected to appear under macro-MAP@10, where minority
class queries—for which hard negatives from the confusable partner class
provide stronger gradient signal—are weighted equally to the majority.
Test-set macro evaluation of the random-mining checkpoint is pending.
...
Validation micro-MAP@10 shows a slight upward trend across qubit counts
($N{=}4$: 0.879, $N{=}8$: 0.879, $N{=}12$: 0.882), consistent with a
marginal Hilbert-space benefit at $N{=}12$, though all values are within
noise.
Whether this trend extends to macro-MAP@10 remains an open question pending
test-set evaluation of the A5 checkpoints.
...
classes.
Immediate extensions include test-set macro-MAP@10 evaluation of all
A3--A5 ablation checkpoints (training complete; macro evaluation pending),
