...
outperforming both the parameter-matched classical head and the
SLIQ-style sequential encoding baseline on all three metrics.
The macro-MAP gap versus SLIQ-style encoding ($\Delta = 0.061$) is
notably larger than the gap versus the classical head ($\Delta = 0.005$),
isolating the encoding strategy—not the quantum circuit itself—as
the primary driver of minority-class retrieval improvement.
\end{abstract}
...
split for all three models.
\qts{} achieves micro-MAP@10 = 0.879, macro-MAP@10 = 0.533,
Recall@1 = 0.856, and Recall@5 = 0.951.
The macro-MAP gap versus SLIQ-style sequential encoding ($\Delta = 0.061$)
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    ResNet50 (strong baseline)    & \res{pend.}    & \res{pend.}    & \res{pend.}    & \res{pend.}    \\
  ...
  regardless of frequency. ResNet50 training in progress.
\end{table}
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    \qts{}'s interleaved encoding yields the fewest confusions between
    visually similar pairs, consistent with the macro-MAP@10 gap in
    Table~\ref{tab:main}.%
  }
  \label{fig:confusion}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    for all three models, colored by terrain class.
    \qts{} (right) shows tighter clustering of minority classes and
    clearer separation between the confusable dune pair
...
micro-MAP@10 a misleading primary metric (all three models score above
0.86 on micro-MAP, masking large differences on minority classes).
...
incorporating the ResNet50 strong baseline,
