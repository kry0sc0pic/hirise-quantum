...
We present a five-factor ablation study on HiRISE~v3 (73,031 patches,
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    ...
    \textbf{\qts{} (ours)}        & \res{\bf X.XX}& \res{\bf X.XX}& \res{\bf X.XX}\\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  \\\footnotesize\textbf{[Fill in experimental results before submission.]}
\end{table}
...
\begin{figure}[t]
  \centering
  \fbox{\rule{0pt}{3.2cm}\rule{\linewidth}{0pt}}
  \caption{%
    ...
    $N=8$ qubits.
    \textbf{[Replace with actual gradient norm figure.]}%
  }
  \label{fig:gradnorm}
\end{figure}
...
\begin{figure}[t]
  ...
  \fbox{\rule{0pt}{3.5cm}\rule{\linewidth}{0pt}}
  \caption{%
    UMAP projection of test-set encoder embeddings (5{,}000 samples).
    Left: Classical head.
    Right: \qts{} (ours).
    Colors indicate terrain class (see Table~\ref{tab:dataset}).
    Qualitative cluster separation between confusable dune classes
    and between spider/swiss\_cheese indicates the quantum head's
    improved discrimination on hard pairs.
    \textbf{[Replace with actual UMAP figure.]}%
  }
  \label{fig:umap}
\end{figure}
...
This is approximately \res{$X\times$} slower per epoch than the classical
head on identical hardware.
...
task.
A five-factor ablation study provides a controlled account of each design
decision.

\qts{} opens a research thread at the intersection of quantum machine
learning and planetary science.
Immediate extensions include running ablation experiments to completion
and reporting the numerical results deferred here, scaling to $N>12$
via hardware backends or tensor-network simulation, and extending the
retrieval system to multi-temporal change detection across HiRISE
revisit sequences.
