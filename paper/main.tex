...
\begin{table}[t]
  ...
  Test-set macro evaluation of A3/A4/A5 checkpoints pending.
  A4 random-mining and A5 $N{=}12$ runs pending.
\end{table}

\textbf{Preliminary A3/A5 findings.}
Based on best-epoch validation micro-MAP@10 (see Table~\ref{tab:ablation}
footnote), circuit depth (A3) has negligible effect on retrieval performance:
$L=1$ (0.882), $L=2$ (0.879), and $L=3$ (0.883) are within 0.4\% of each
other, suggesting that a single entangling layer is sufficient at $N=8$
qubits to capture the pairwise similarity structure learned from triplet loss.
Qubit count (A5) shows similarly flat behavior between $N=4$ (0.879) and
$N=8$ (0.879), implying that the embedding dimensionality bottleneck lies in
the CNN projection head rather than the quantum Hilbert space size.
Test-set macro-MAP@10 evaluation of A3/A5 checkpoints is pending; A4
(hard vs.\ random negatives) and A5 $N{=}12$ runs are also pending.
...
hardware; a precise speedup ratio will be reported alongside the A4
ablation run.
...
Preliminary results show that $N=4$ and $N=8$ achieve identical micro-MAP@10
(0.879), suggesting that the retrieval bottleneck at this scale lies in the
CNN projection dimensionality rather than the Hilbert space size.
Whether larger qubit counts provide an advantage under macro-MAP@10—which
penalizes minority-class errors more heavily—remains an open question pending
test-set evaluation of the $N=4$ checkpoint and the $N=12$ run.
...
(random-negative mining A4, qubit count $N{=}12$ A5, and test-set macro
evaluation of A3/A5 checkpoints already trained),
